...
class Rook < Piece
  ...
  def self.legal_moves_up(src, game, letter)
    result = []
    (1..7).each do |i|
  ...
  def self.legal_moves_right(src, game, letter)
    result = []
    (1..7).each do |i|
  ...
  def self.legal_moves_down(src, game, letter)
    result = []
    (1..7).each do |i|
      if !game.board.valid_coords?([src[0] + i, src[1]])
        break
  ...
  def self.legal_moves_left(src, game, letter)
    result = []
    (1..7).each do |i|
      if !game.board.valid_coords?([src[0], src[1] - i])
        break
